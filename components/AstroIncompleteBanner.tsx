...
export function AstroIncompleteBanner({ onSuccess }: Props) {
  ...
  const retrigger = async () => {
    ...
    try {
      ...
      if (msg === 'no_birth_data') {
        setErrorMsg('Birth details not found. Please re-enter them below.');
  ...
  return (
    <View style={styles.banner}>
      ...
      <Text style={styles.body}>
        Your birth details were saved but the astrological computation failed.
        Compatibility scores on the feed are showing as 0% until this is fixed.
      </Text>
