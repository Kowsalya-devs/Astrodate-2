...
import { getSynastryDetail, derivedAstroScore, type SynastryDetail } from '@/lib/synastry';
...
interface PlanetBarProps {
  ...
  emoji: string;
  locked: boolean;
  teaserLabel?: string;    // e.g. "High Passion" — shown blurred for free users
}

const SCORE_COLOR = (score: number) =>
  score >= 8 ? '#f59e0b' : score >= 6 ? '#8b5cf6' : score >= 4 ? '#3b82f6' : '#6b7280';
...
function PlanetBar({ label, score, emoji, locked, teaserLabel }: PlanetBarProps) {
    ...
    return (
      <View style={{ marginBottom: 12 }}>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 4 }}>
          <Text style={{ color: 'rgba(255,255,255,0.7)', fontSize: 13 }}>{emoji} {label}</Text>
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            <Text style={{ fontSize: 13, marginRight: 4 }}>🔒</Text>
            {/* Blurred score label — curiosity gap */}
            <BlurView intensity={18} tint="dark" style={{ borderRadius: 6, overflow: 'hidden' }}>
              <Text style={{ color: '#fff', fontSize: 12, fontWeight: '700', paddingHorizontal: 6, paddingVertical: 2 }}>
                {teaserLabel ?? 'Hidden'}
              </Text>
            </BlurView>
          </View>
        ...
        <View style={{ height: 6, borderRadius: 3, backgroundColor: 'rgba(255,255,255,0.1)', overflow: 'hidden' }}>
          ...
          <View style={{ width: '60%', height: '100%', backgroundColor: '#8b5cf6', borderRadius: 3, opacity: 0.3 }} />
        </View>
      </View>
    );
  }
  ...
  return (
    <View style={{ marginBottom: 12 }}>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 4 }}>
        <Text style={{ color: 'rgba(255,255,255,0.85)', fontSize: 13 }}>{emoji} {label}</Text>
        <Text style={{ color: SCORE_COLOR(score), fontSize: 13, fontWeight: '700' }}>
          {score.toFixed(1)} / 10
        </Text>
      </View>
      <View style={{ height: 6, borderRadius: 3, backgroundColor: 'rgba(255,255,255,0.12)' }}>
        <View style={{ width: barWidth, height: '100%', borderRadius: 3, backgroundColor: SCORE_COLOR(score) }} />
      </View>
    </View>
  );
}

interface SynastrySectionPanelProps {
...
function SynastrySectionPanel({ detail, loading, isPremium, onUpgradePress }: SynastrySectionPanelProps) {
  if (loading) {
    return (
  ...
  const teaserLabels: Record<string, string> = {
    venus_score:   detail.venus_score   >= 7 ? 'High Passion'   : detail.venus_score   >= 5 ? 'Warm Energy' : 'Gentle Vibes',
    mars_score:    detail.mars_score    >= 7 ? 'Strong Drive'   : detail.mars_score    >= 5 ? 'Compatible'  : 'Laid Back',
    mercury_score: detail.mercury_score >= 7 ? 'Deep Bond'      : detail.mercury_score >= 5 ? 'Good Flow'   : 'Different Styles',
  };
    ...
    <View style={{ marginHorizontal: 16, marginTop: 20 }}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 16 }}>
        ...
        <View style={{
          ...
          borderWidth: 3,
          borderColor: compositeScore >= 75 ? '#f59e0b' : compositeScore >= 50 ? '#8b5cf6' : '#6b7280',
          justifyContent: 'center', alignItems: 'center',
          backgroundColor: 'rgba(255,255,255,0.06)',
        }}>
          <Text style={{ color: '#fff', fontWeight: '800', fontSize: 18 }}>{compositeScore}</Text>
          <Text style={{ color: 'rgba(255,255,255,0.55)', fontSize: 8 }}>AstroScore</Text>
        </View>
      </View>

      {/* Compatibility summary */}
      {detail.compatibility_summary ? (
        <Text style={{ color: 'rgba(255,255,255,0.8)', fontSize: 14, lineHeight: 20, marginBottom: 16 }}>
          {detail.compatibility_summary}
        </Text>
      ) : null}
      ...
      {detail.badges && detail.badges.length > 0 && (
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 16 }}>
          {detail.badges.map((badge) => (
            <View key={badge} style={{ backgroundColor: 'rgba(139,92,246,0.25)', borderRadius: 20, paddingHorizontal: 10, paddingVertical: 4 }}>
      ...
      <PlanetBar label="Venus"   emoji="💕"  score={detail.venus_score}   locked={!isPremium} teaserLabel={teaserLabels.venus_score} />
      <PlanetBar label="Mars"    emoji="🔥"  score={detail.mars_score}    locked={!isPremium} teaserLabel={teaserLabels.mars_score} />
      <PlanetBar label="Mercury" emoji="☿️"  score={detail.mercury_score} locked={!isPremium} teaserLabel={teaserLabels.mercury_score} />

      {/* Element match indicator */}
      {detail.dominant_element_match && (
        <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 4 }}>
          <Text style={{ fontSize: 14, marginRight: 6 }}>🌿</Text>
          <Text style={{ color: 'rgba(255,255,255,0.7)', fontSize: 13 }}>Matching dominant elements</Text>
          <Text style={{ color: '#34d399', fontWeight: '700', fontSize: 13, marginLeft: 'auto' }}>✓ Aligned</Text>
        </View>
      )}

      {/* Premium upsell teaser (free users only) */}
      {!isPremium && (
        <TouchableOpacity
        ...
        >
          <LinearGradient
            ...
            style={{ padding: 14 }}
          >
            <Text style={{ color: '#c4b5fd', fontWeight: '700', fontSize: 14, marginBottom: 4 }}>
              🔒 Your Venus signs show intense passion, but there's a catch…
            </Text>
            <Text style={{ color: 'rgba(255,255,255,0.6)', fontSize: 12, lineHeight: 17 }}>
              Upgrade to reveal your full Deep Synastry — Venus, Mars, and Mercury tell the real story of romantic chemistry and communication style.
            </Text>
            <View style={{ marginTop: 10, alignSelf: 'flex-start', backgroundColor: '#8b5cf6', borderRadius: 20, paddingHorizontal: 14, paddingVertical: 6 }}>
              <Text style={{ color: '#fff', fontWeight: '700', fontSize: 13 }}>Unlock Deep Synastry ✨</Text>
            </View>
          </LinearGradient>
        </TouchableOpacity>
      )}
...
export default function ProfileDetailsScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        console.warn('[profile-details] Synastry fetch failed:', e);
      } finally {
  ...
  const checkAndShowMatch = async (likedUserId: string): Promise<boolean> => {
    try {
      console.log('🔍 [MatchModal] Checking for mutual like with user:', likedUserId);
      const result = await checkMutualLike(likedUserId);
      if (result.isMatch) {
        console.log('💕 [MatchModal] Match found! Channel ID:', result.channelId);
        ...
        console.log('✅ [MatchModal] Modal state set to visible');
        return true; // Signal that match was found
      }
      console.log('❌ [MatchModal] No match found');
      return false; // Signal no match
    } catch (error) {
      console.error('❌ [MatchModal] Error checking for match:', error);
      return false;
    }
  };
  ...
  const handleSendMessageFromMatchModal = async () => {
    ...
    console.log('💬 [MatchModal] Send message clicked, navigating to chat with:', chatUserId);
    setShowMatchModal(false);
    setMatchedProfile(null);
    setMatchedUserId(null);
    console.log('✅ [MatchModal] Modal state reset (send message)');
  ...
  const handleLike = async () => {
    ...
    try {
      ...
      if (result.success) {
        // Use the return value from checkAndShowMatch instead of stale state
...
const handleSuperLike = async () => {
  ...
  // If opened from likes/superlikes tab, go directly to chat
    ...
    >
      ...
      <View style={styles.contentWrapper}>
        ...
        >
          ...
          <SynastrySectionPanel
            detail={synastryDetail}
            loading={synastryLoading}
            isPremium={isPremiumUser}
            onUpgradePress={() => router.push('/settings')} // adjust to your paywall screen
          />
  ...
  <View style={styles.actionIconsContainer}>
    <TouchableOpacity activeOpacity={0.7} style={styles.actionButtonSmallWrapper} onPress={handleSuperLike}>
      ...
      <Modal
        ...
        animationType="fade"
        onRequestClose={() => {
          console.log('🔙 [MatchModal] Modal onRequestClose triggered (e.g., Android back button)');
          setShowMatchModal(false);
          setMatchedProfile(null);
          setMatchedUserId(null);
          console.log('✅ [MatchModal] Modal state reset (onRequestClose)');
        }}
      >
        {matchedProfile && (
          <LinearGradient
          ...
          >
            ...
            <Pressable
              ...
              onPress={() => {
                console.log('❌ [MatchModal] Close (X) button pressed');
                setShowMatchModal(false);
                setMatchedProfile(null);
                setMatchedUserId(null);
                console.log('✅ [MatchModal] Modal state reset (close button)');
                router.back();
              }}
